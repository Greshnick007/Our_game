...
// Костыыыыль мляяяяяя, но пофиг))))
var Igrok = new Image();
Igrok.src = "img/Pistol_0.png";

var Shot_POST = false; //костыль для проверки зажатия клавыши на прошлом рендере. Т.е. для смены изображения стрельбы
...
class Statistic {
    ...
    addMissed(arg) {
		...
		var pos_enemy_1 = new Image();
		pos_enemy_1.src = 'img/1.png';
		var pos_enemy_2 = new Image();
		pos_enemy_2.src = 'img/2.png';
		var pos_enemy_3 = new Image();
		pos_enemy_3.src = 'img/3.png';
		var pos_enemy_4 = new Image();
		pos_enemy_4.src = 'img/4.png';
		var pos_enemy_5 = new Image();
		pos_enemy_5.src = 'img/5.png';
...
class Enemy {
    ...
    draw() {
        ...
        context.beginPath();
        // Тело
		if (enemy_d.getTime()-this.time_enemy >= 180) {this.time_enemy = enemy_d.getTime(); this.anim_enemy++;}
		if (this.anim_enemy==6) {this.anim_enemy = 1;}
		if (this.anim_enemy == 1) {context.drawImage(pos_enemy_1, this.x, this.y);}
		if (this.anim_enemy == 2) {context.drawImage(pos_enemy_2, this.x, this.y);}
		if (this.anim_enemy == 3) {context.drawImage(pos_enemy_3, this.x, this.y);}
		if (this.anim_enemy == 4) {context.drawImage(pos_enemy_4, this.x, this.y);}
		if (this.anim_enemy == 5) {context.drawImage(pos_enemy_5, this.x, this.y);}
        context.closePath();
        ///////
    }
    ...
    // Попали ли по врагу
    isHit(x, y) {
        if( (x < this.x+120) && (x > this.x) ) {
            if( (y < this.y+85) && (y > this.y) ) {
                return true;
            }
        }
...
class Player {
    constructor() {
        ...
        this.x = canvas.width - 300; // Положение на холсте
    ...
    draw() {
		...
		if (weapons[0].isActive == true && Shot_POST == false) {Igrok.src = "img/Pistol_0.png";}
		if (weapons[1].isActive == true && Shot_POST == false) {Igrok.src = "img/Ralfe_0.png";}
		if (weapons[2].isActive == true && Shot_POST == false) {Igrok.src = "img/Drob_0.png";}
		if (weapons[0].isActive == true && Shot_POST == true) {Igrok.src = "img/Pistol_1.png";}
		if (weapons[1].isActive == true && Shot_POST == true) {Igrok.src = "img/Ralfe_1.png";}
		if (weapons[2].isActive == true && Shot_POST == true) {Igrok.src = "img/Drob_1.png";}
		context.drawImage(Igrok, this.x, this.y);
        context.closePath();
...
function user_interface() {
    ...
    context.strokeText('Выстрелов: '+player.stat.missed, 900, canvas.height-35);
    context.closePath();
}
